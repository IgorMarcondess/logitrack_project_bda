...
CREATE OR REPLACE PACKAGE entrega_pkg AS
    -- Funções
    FUNCTION verificar_cliente(v_id_cliente NUMBER) RETURN BOOLEAN;
    FUNCTION verificar_entrega(v_id_entrega NUMBER) RETURN BOOLEAN;

    -- Procedures
    PROCEDURE registrar_entrega(v_id_clie NUMBER, v_tipo_entrega VARCHAR2);
    PROCEDURE atribuir_veiculo_entrega(v_id_entrega NUMBER, v_tipo_entrega VARCHAR2);
    PROCEDURE verificar_status(v_id_entrega NUMBER);

END entrega_pkg;

CREATE OR REPLACE PACKAGE BODY entrega_pkg AS
    ----> FUNCTIONS

    FUNCTION verificar_cliente(v_id_cliente NUMBER)
        RETURN BOOLEAN IS
        v_count NUMBER;
    BEGIN
    ...
    FUNCTION verificar_entrega(v_id_entrega NUMBER)
        RETURN BOOLEAN IS 
        v_count NUMBER;
    ...
    PROCEDURE atribuir_veiculo_entrega (
        v_id_entrega     NUMBER,
        v_tipo_entrega   VARCHAR2
    ) IS
        v_count NUMBER;
